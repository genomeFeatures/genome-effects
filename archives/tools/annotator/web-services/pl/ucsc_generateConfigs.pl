...
# Cehck ecode annotations here :http://www.gencodegenes.org/mouse_releases/
#
...
#
#   d. ucsc_features.txt with the following fields:
#       1.organism_group,
#       2.organims_name, 
#       3.ucsc_db, 
#       4.feature tables list separated by ":"
...
my $response = $browser->get($host, ":content_file" => $filename); open(IN,"$filename");
 ...
 system("rm $filename");my %organismmap;
$version_file="ucsc_organism_version.txt";$genome_file="ucsc_organisms_w_chromosome_data.txt";
$annot_file="ucsc_geneannotations.txt"; $features_file="ucsc_features.txt";
if($directory){
   $version_file="$directory/ucsc_organism_version.txt";$genome_file="$directory/ucsc_organisms_w_chromosome_data.txt";
   $annot_file="$directory/ucsc_geneannotations.txt";$features_file="$directory/ucsc_features.txt";
}open(O,">$version_file");open(G,">$genome_file");open(A,">$annot_file");open(F,">$features_file");
while(@organismgroups){
      $organismg=shift(@organismgroups); $newurl="$host"."&clade=$organismg";
      my $filename="organismList".".txt"; my $response = $browser->get($newurl, ":content_file" => $filename);
      open(IN,"$filename");if(IN){@filecontent=<IN>;}close(IN);$data="";$more=1;
      while($more){ $line=shift(@filecontent);chomp($line);
          ...
          if($line=~/VALUE="(.+)">(.+)<\/OPTION>/){
             ...
             while(@dbs>0){$db=shift(@dbs);
                  if($db=~/VALUE="(.+)">(.+)<\/OPTION>/i){$db=$1;$desc=$2;
                     $data="";$more=1;$commands=("$connection -e 'use $db; show tables' > $filename");
                     system($commands);open(IN4,"$filename");@filecontent=<IN4>;#close(IN4);
                     while(<IN4>){chomp($_);push(@filecontent,$_);}close(IN4);
                     @tablelist=();@tablelist=grep(/Gene\s*$/,@filecontent);@featurelist=();
                     @encodes= grep(/^\s*wgEncodeGencodeComp.+$/i,@filecontent);@acembly=grep(/^\s*acembly\s*$/,@filecontent);
                     if(@encodes>0){push(@tablelist,@encodes);}if(@acembly>0){push(@tablelist,@acembly);}
                     #genome features  @gap=grep(/^\s*gap\s*$/,@filecontent); 
                     @snps=grep(/^\s*snp\d+\s*$/,@filecontent);@repeats=grep(/^\s*simpleRepeat\s*$/,@filecontent);
                     @miRNA=grep(/^\s*miRNA\s*$/,@filecontent);@microsats=grep(/^\s*microsat\s*$/,@filecontent);
                     if(@gap>0){push(@featurelist,@gap);}if(@snps>0){push(@featurelist,@snps);}
                     if(@repeats>0){push(@featurelist,@repeats);}if(@miRNA>0){push(@featurelist,@miRNA);}
                     if(@microsats>0){push(@featurelist,@microsats);}
                     $newurl="$ftp_path/$db";
                     my $response = $browser->get($newurl, ":content_file" => $filename);
                     open(IN4,"$filename");@filecontent=();@filecontent=<IN4>;close(IN4);
                     ...
                     if(@tablelist>0){ @temps=(); foreach $table(@tablelist){chomp($table);push(@temps,$table);}
                         print A "$organismg,$organism,$db,".join(":",@temps)."\n"; 
                     }
                     if(@featurelist>0){ @temps=();$snp_version=0;$table_tok="";
                        foreach $table(@featurelist){chomp($table);
                            if($table=~/snp(\d+)\s*/){if($snp_version<$1){$snp_version=$1;$table_tok=$table;}}
                            else{push(@temps,$table);}
                        }if($table_tok ne ""){push(@temps,$table_tok);}
                         print F "$organismg,$organism,$db,".join(":",@temps)."\n"; 
                     }
                     #connect to uniprot db for the following queries
                    # $commands=("$connection -e 'use uniProt; show tables' > $filename");
                    # system($commands);open(IN4,"$filename");@filecontent=();@filecontent=<IN4>;
                    # while(<IN4>){chomp($_);push(@filecontent,$_);}close(IN4);
                       # @extDbRef=grep(/^\s*extDbRef\s*$/,@filecontent);
                       # @featureType=grep(/^\s*featureType\s*$/,@filecontent);
