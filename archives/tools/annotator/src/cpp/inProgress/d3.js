...
d3.behavior.zoom = function() {
  var translate = [0, 0],
      ...
      lastTouch; // location of last touch (for double tap)
  ...
  function touchstart() {
    ...
    if (touches.length === 1) {
      if (now - touchtime < 500 && (touches[0] * touches[0] - lastTouch[0] * lastTouch[0]) + (touches[1] * touches[1] - lastTouch[1] * lastTouch[1]) < 100) { // dbltap: dt < .5s, dp < 10px
  ...
  function touchmove() {
    if(translate0) {console.log(document.activeElement);}
    var touches = d3.touches(this),
        p0 = touches[0],
