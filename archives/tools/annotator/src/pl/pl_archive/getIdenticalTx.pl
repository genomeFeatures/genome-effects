...
sub getTranscript{
 ...
 $qh_getTxGeneAll->execute($org_vid,$tx_id);$gene="";
  ...
  while(($tx_exons,$txs)=each(%tx_map)){
   ...
   $cds_ary_ref=$dbh->selectall_arrayref($query) or die "$query -- ".$dbh->errstr;$cds_count=0;
   foreach $cds(@{$cds_ary_ref}){($cdsStart,$cdsEnd)=@{$cds}; $cdsStart+1;
           next if(($cdsEnd-$cdsStart)<=1);
           $cdsStarts=($cdsStarts eq "")? "$cdsStart":"$cdsStarts,$cdsStart";$cdsEnds=($cdsEnds eq "")?"$cdsEnd":"$cdsEnds,$cdsEnd";
           ++$cds_count;
    }@exon_count=split(",",$tx_exons); 
