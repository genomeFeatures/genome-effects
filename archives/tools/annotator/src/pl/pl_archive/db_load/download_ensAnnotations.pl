...
use DBI;use vars qw ($opt_h $opt_a $opt_o $opt_d);
...
Examples:
cmd: ./download_ensAnnotations.pl -o $basedir -a $gene_prediction -v $oversion 
...
$oversion=$opt_v;$gene_prediction=$opt_a;
$versionid=$versionmap{"$oversion"};$core=$schema_core{"$oversion"};

@mus=grep(/$core\d+_$versionid/,@databases);$dbid=0;$eschema="";
foreach $db(@mus){ chomp($db);
  $db=~/$core(\d+)_$versionid/;if($dbid<$1){$eschema=$db;}
}
