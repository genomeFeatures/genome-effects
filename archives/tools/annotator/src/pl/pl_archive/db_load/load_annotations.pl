...
cmd: ./load_Annotations.pl -d /scratch/data/downloads/ensembl/mm9/mus_musculus_core_70_38 -f /scratch/data/downloads/ensembl/mm9/mus_musculus_core_70_38/GRCm38-ensGene.txt -a GRCm38-ensGene -v mm10  -s 1 -e 1
HELP
exit;
}
...
my $qh_drop_dates=$dbh->prepare("drop temporary table if exists rnaseq_sample_date_temp");
my $qh_create_dates=$dbh->prepare("create temporary table if not exists rnaseq_sample_date_temp(sample_name varchar(50),found tinyint default 0)");
my $qh_loadDates=$dbh->prepare("load data local infile ? into table sample_temp (sample_name)");
my $qh_update_dates=$dbh->prepare("sample_temp  t, rnaseq_sample r set found=1 where t.sample_name=r.sample_name");
my $qh_insert_dates=$dbh->prepare("insert into rnaseq_sample(sample_name) select distinct sample_name from sample_temp where found=0");
########### addon ends #########
...
my $insert_predict_id="insert into gene_prediction_by_organism values(?,?, concat(CURDATE(),':',CURTIME()))";
...
my $insert_ta="insert into transcript_by_annotation(organism_version_id,transcript_name,transcript_id,gene_prediction_id) ";
...
my $insert_tl="insert into transcript_translation(organism_version_id,transcript_name,transcript_id,gene_prediction_id,cdsStart,cdsEnd) ";
...
my $insert_te="insert into transcript_exon(organism_version_id,transcript_name,transcript_id,exon_id,gene_prediction_id,exon_frame) ";
...
my $insert_ga="insert into gene_by_annotation(organism_version_id,gene_name,transcript_id,gene_prediction_id)";
...
my $insert_ea="insert into ensembl_exons(exon_id,organism_version_id,accession_id )";
...
if($opt_a=~/DOmiceRNAseqRUM/){#rna seq annotations from Nazia
  $sample_file="$opt_d/samples.txt";
  $qh_drop_sample->execute();$qh_create_sample->execute();
  $qh_loadSample->execute($sample_file);$qh_update_sample->execute();$qh_insert_sample->execute();
  ##get ssample id, get dates id
}
if(LOG){
  $tm = localtime;
  ...
  $update_table="$oversion-$gene_prediction"; ($orgv_id)=$qh_orgvlist->fetchrow_array();
  $qh_get_predict_id->execute($gene_prediction);($prediction_id)=$qh_get_predict_id->fetchrow_array();
  ...
  if(-f "$text_file"){ $rowcount=0;$linecont=0;
         ...
         $exon_ids="$opt_d/tempfile-$gene_prediction-exonids.txt"; 
         $transcript_by_annotation="$opt_d/tempfile-$gene_prediction-transcript_by_annotation.txt";        
         $transcript_translation="$opt_d/tempfile-$gene_prediction-transcript_translation.txt";
         $transcript_exon="$opt_d/tempfile-$gene_prediction-transcript_exon.txt";
         $gene_by_annotation="$opt_d/tempfile-$gene_prediction-gene_by_annotation.txt";
         $est_align="$opt_d/tempfile-$gene_prediction-est_align.txt";
         $est_orientation="$opt_d/tempfile-$gene_prediction-est_orientation.txt";
         ...
         if(FH){
            ...
            while($line=<FH>){ chomp($line);
                ...
                if($opt_a=~/estOrientInfo/i){ #est_all table
                   ...
                   @exonIdss=split(",",$exonIds);
                   if(@startExons==@endExons){
                      while(@startExons>0){
                        $exonStart=shift(@startExons);$exon_frame=-1;$eid="";
                        if(@endExons>0){$exonEnd=shift(@endExons);$exon_frame=shift(@exonFrames);$eid=shift( @exonIdss);}
                 ...
                 if($name2 ne ""){
                    print GA "$trans_id\t$name2\t$chr_id\t$strand\t$txStart\t$txEnd\t$prediction_id\t$orgv_id\n";
                 }
         ...
         if($linecont>0){
           ...
           ($rowcount)=$qh_get_tl_rowcount->fetchrow_array();
            if($linecont==$rowcount){$qh_update_tl_temp->execute();
               $qh_delete_tl_temp->execute($orgv_id,$prediction_id);$qh_insert_tl->execute();
         ...
         if($linecont>0){
           ...
           ($rowcount)=$qh_get_te_rowcount->fetchrow_array();
            if($linecont==$rowcount){$qh_update_te_temp->execute();
              $qh_update_te_e_temp->execute();$qh_delete_te_temp->execute($orgv_id,$prediction_id);$qh_insert_te->execute();
         ...
         if($linecont>0){
           ...
           ($rowcount)=$qh_get_ea_rowcount->fetchrow_array();
            if($linecont==$rowcount){$qh_update_ea_temp->execute();
              $qh_delete_ea_temp->execute($orgv_id);$qh_insert_ea->execute();
         ...
         if($linecont>0){
           ...
           ($rowcount)=$qh_get_ga_rowcount->fetchrow_array();
            if($linecont==$rowcount){$qh_update_ga_temp->execute();
              $qh_delete_ga_temp->execute($orgv_id,$prediction_id);$qh_insert_ga->execute();
     ...
     $qh_insert_log->execute($update_table,$text_file,$linecont,$rowcount);  
