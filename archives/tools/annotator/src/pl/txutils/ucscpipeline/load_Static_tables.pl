...
my $config_path="$load_base_path/docs";
#my $sql_path="$load_base_path/src/sql";
my $data_path="$load_base_path/currenttables";
my %annotmap;
$tm = localtime;
...
if(LOG){
  chomp($downloadedAnnot_config);chomp($orgv_configFile);chomp($annoFortmatconfig);
   ...
   open(OGV,"$orgv_configFile");open(DL,"$downloadedAnnot_config");open(ANO,"$annoFortmatconfig");
   if(!OGV ||!DL ||!ANO){ #if one of the config files has issues report and quit
      $log="Can not open the following file(s) :\n";
      $log.="$orgv_configFile," if(!OGV); $log.="$downloadedAnnot_config," if(!DL);
      $log.="$annoFortmatconfig" if(!ANO);
      print LOG "$log\n";
      close(LOG); exit(1);
   }
...
## chromosome load
  ...
  while(@row=$qh_gannotlist->fetchrow_array()){
   ...
   while($line=<ANO>){ #ensGene:horse:mammal:equCab2:16,
            ...
            if(($orgv_id<=0)&&($ucsc_db=~/\w+/)){
             ...
             for my $annot_name(sort keys (%{$annot_map{$neworg}})){
                     ...
                     if(($annot_id<=0)&&($annot_name=~/\w+/)){
                         ...
                         if(@row=$qh_gtgannot->fetchrow_array()){
                            $annot_id=$row[0];
             ...
             if(-f $chrominfo_file){       #we are loading the chromosome info for this organism
                 ...
                 if(CHR){
                      ...
                      if($annoFortmatconfigmap{$chrominfo}{lc($organism)}{$ucsc_db}=~/,chrom>(\d+)/i){$chromindex=$1;}
                      if($annoFortmatconfigmap{$chrominfo}{lc($organism)}{$ucsc_db}=~/,fileName>(\d+)/i){$fileindex=$1;}
                      if($annoFortmatconfigmap{$chrominfo}{lc($organism)}{$ucsc_db}=~/,size>(\d+)/i){$sizeindex=$1;}
