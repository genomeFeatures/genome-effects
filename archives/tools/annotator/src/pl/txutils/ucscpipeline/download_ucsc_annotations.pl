...
$config=shift||"db_svn/docs/prog_generated_config.txt";
chomp($config);
open(CONF,"$config");
if(!CONF){
...
my $configpath="$load_base_path/docs";
...
if(LOG){
  ...
  if(!$configFile){
   ...
   if(IN){ @organisms=<IN>;
      foreach my $line(@organisms){next if(!($line=~/,/));chomp($line); 
         my($organism_group,$organism,$db,$tables)=split(",",$line);
         chomp($organism);#next if(!($line=~/C\.elegans,|D\.melanogaster,|Zebrafish,/));
         $organism=lc($organism);$organismDir=$organism;$organismDir=~s/\s+/-/g; 
         if(!(-d "$path/$organism_group")){mkdir("$path/$organism_group",0777)||print LOG "$!\n";}
         if(!(-d "$path/$organism_group/$organismDir")){ mkdir("$path/$organism_group/$organismDir",0777)||print LOG "$!\n";}
         if((-d "$path/$organism_group")&& (-d "$path/$organism_group/$organismDir")){
            ...
            while(@tablelist>0){$table=shift(@tablelist);
                 if($table){
                    next if($table=~/sex|source|library|mrnaClone|tissue|development/);
                    next if($table=~/cell|description|geneName|author|gbCdnaInfo/);
                    if(!($table=~/refFlatSquishMulti/)){
                         ...
                         if(-f "$path/$organism_group/$organismDir/archive/$file"){
                            $lines=`wc -l $temp_file`;$linecount=0;$lines=~s/^\s+//; #check if the file is empty
                         ...
                         if(!(-f $filename)){$fields="*";$where="";
                            ...
                            $commands="$connection --quick -e 'use $db;";
                            $commands.=" select $fields from $table  $where' > $filename";
