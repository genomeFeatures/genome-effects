...
if(($opt_h)|| !($opt_d)){
    ...
    **************************************
    ...
    Script usage: ./loadNovelTranscripts.pl -d filesDir
    where filesDir is a directory where novel transcripts files are stored.
    example:  ./loadNovelTranscripts.pl -d CC_to_database
...
my $query="select MAX(cast(substring(transcript_name from locate('rsd_',transcript_name)+4) as unsigned)) ";
$query.=" from transcript_by_annotation WHERE transcript_name like 'rsd_%'";
my $qh_getCurrentMaxRsdNumber= $dbh->prepare($query);
my $qh_getnovelTxid=$dbh->prepare("select distinct transcript_name from transcript_by_annotation where transcript_id=? and gene_prediction_id=?");
my $qh_getPredId=$dbh->prepare("select gene_prediction_id from gene_prediction where gene_prediction_name in (?)");
...
my $qh_getexons=$dbh->prepare($query);
...
my $qh_getTxRsds=$dbh->prepare($query);
...
my $qh_getTxCord=$dbh->prepare($query);
#load rnaseq_transcript table
my $qh_loadRnaSeqTx=$dbh->prepare("load data local infile ? into table rnaseq_transcript ignore 1 lines");
$query="select count(*) from rnaseq_transcript where organism_version_id=? and gene_prediction_id=?";
my $qh_getRowCount=$dbh->prepare($query);
...
sub loadCurrentTxRsd{
  ...
  my ($exonStarts,$exonEnds,$tx_id,$exon_start,$exon_end,$transcript_id,$key,$tx_name,$chrom,$strand,$txstart,$txend);
...
@files=();@files=`ls $opt_d`;
if(@files==0){ print LOG "Failed to execute the command 'ls $opt_d': $!\n"; close(LOG);exit;}
...
foreach $file(@files){chomp($file); 
   ($orgv,$source,$sample,$desc,$suffix)=split(/\./,$file);next if(!($source=~/Gene/));
    $strainFiles{"$source"}{"$sample"}=$file;$orgversion=$orgv if($suffix);
 }
...
while(($prediction,$samplefiles)=each(%strainFiles)){
    ...
    while(($sample,$file)=each(%{$samplefiles})){ 
         ...
         if($chromindex==-1||$strandindex==-1||$txStartindex==-1||$txEndindex==-1||
            ...
            print LOG "$file -- Bad header line for a genePrediction file format-\n"; next;
         }#Index transcriptome
   ...
   } #Now integrate these annotations in our database 
   $linecount = `wc -l $annotFile`;chomp($linecount); 
  if($linecount=~/^(\d+)\s$annotFile/){$linecount=$1;--$linecount;
     if($linecount>0){
       $cmd="perl $cwdir/load_annotations.pl -d $cwdir -f $annotFile -a $prediction -v $orgversion -s 0 -e 1";
       system($cmd);if($? !=0){print LOG "Failed to execute $cmd\n";}
      }
   }
  #process next strain
 }# end of 
