...
my $getTxExons="select distinct chromosome_name,exon_start,exon_end,strand,exon_frame 
   ...
   $getTxExons.=" where transcript_id=? and transcript_name=? and t.organism_version_id=?
           and gene_prediction_id in (?)";
...
my $getTxGene="select distinct gene_name from gene_by_annotation g, transcript_by_annotation t
                where g.organism_version_id=? and transcript_name=? and t.gene_prediction_id in (?)
...
sub getFeatures{
   ...
   $qh_getTxExons->execute($tx_id,$tx,$org_vid,$prediction_id);
   $exoncount=$qh_getTxExons->rows;$prank=1; $nrank=$exoncount;$rank=0; 
   $rows=""; $featureType="";$gene_id="";
   if($qh_getTxExons->rows<=0){return $rows;}
   $qh_getTxGene->execute($org_vid,$tx,$prediction_id);
   $gene_name="";$strand="";$tx_start+=1;
   while(($gene_name)=$qh_getTxGene->fetchrow_array()){
     $gene_id=$gene_name if($gene_name=~/^ENS/);
   }$gene_id=($gene_id eq "")?$gene_name:$gene_id;
   $gene_name=$ccgenemap{"$prediction_id"}{"$gene_id"} if($gene_name eq "");
   #$gene_name($gene_name eq "")? $ccgenemap{"$prediction_id"}{"$gene_id"}:$gene_name;
   $attributes="gene_id \"$gene_id\";transcript_id \"$tx\"; exon_count \"$exoncount\";";
   $attributes.="gene_name \"$gene_name\";" if(($gene_name ne "$gene_id")&&($gene_name ne ""));
   $tstrand="";
   $featureType=($opt eq "lex")?"3'Exon":$featureType;
   $featureType=($opt eq "fex")?"5'Exon":$featureType;
   $featureType=($opt eq "ex")?"exon":$featureType;$sample_id="";
  #get sample ids to fill ther source field
   if($tx=~/^rsd_/){
       $qh_getSampleName->execute($tx,$prediction_id,$org_vid);
       while(($sample)=$qh_getSampleName->fetchrow_array()){
          if($sample_id eq ""){$sample_id=$sample;}else{$sample_id.=",$sample";}
       }
    }$annot=($sample_id eq "")?$annot:$sample_id;
   if(($opt eq "lex")||($opt eq "fex")||($opt eq "tex")){
     ($firstExon,$lastExon)=split(":",getLastTerminalExon($tx_id,$tx,$org_vid,$prediction_id,$opt));
     ($chrom,$fexstart,$fexend,$strand)=split(",",$firstExon);
     ($chrom,$lexstart,$lexend,$strand)=split(",",$lastExon);
     if($opt eq "tex"){
...
if($opt_s){# distinct o.organism_name,ov.organism_version_id ,ucsc_db as version
 ...
 if($tx_qh->rows>0){$count=0;
    while(($tx_id,$tx,$prediction_id)=$tx_qh->fetchrow_array()){
        ...
        if((!$opt_F)&&($type=~/tx/)){$row=getTranscript($tx_id,$tx,$org_vid,$prediction_id);
             print TS "$row"if($row ne "");}
        else{$row=getFeatures($tx_id,$tx,$org_vid,$prediction_id,$type);
        print TS "$row"if($row ne "");}
    }
  }
