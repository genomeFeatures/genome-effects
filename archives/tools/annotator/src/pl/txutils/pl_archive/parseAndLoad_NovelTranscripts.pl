...
use Time::localtime; 
use POSIX; ## set default variables
use Cwd;
...
if(($opt_h)|| !($opt_d)){
    print <<HELP;
    Script usage: program.pl -d filesDir
    where filesDir is a directory where novel transcripts files are stored.
    These files are in genePred format, and the file name have the format: 
...
$query="select distinct chromosome_id,strand,tx_start,tx_end,ga.transcript_id from transcript t, gene_by_annotation ga";
$query.=" where gene_prediction_id=? and ga.organism_version_id=? and ga.gene_name='--' ";
...
sub loadNovelG{
 ...
 $qh_getRcoord->execute($prediction_id,$org_vid);%chromMap=();
  print "A total of ".$qh_getRcoord->rows." novel transcrips on novel genes\n";
 while(($chromosome_id,$strand,$tx_start,$tx_end,$transcript_id)=$qh_getRcoord->fetchrow_array()){
   $key="$chromosome_id:$strand"; $chromMap{"$key"}{$tx_start}{$tx_end}=$transcript_id;
         ...
         if($current_start==0){$current_start=$tx_start;$current_end=$tx_end;$tx_list="$tx_id";}
            ...
            if(($tx_start>$current_start)&&($tx_start<$current_end)){
               ...
               $tx_list.=",$tx_id";$more=1;
                 ...
                 $current_start=$tx_start;$current_end=$tx_end;$tx_list="$tx_id";$more=0;
             }
         }
      }
      ##store last
...
sub getOverlapGenes{
  ...
  while(($key,$data)=each(%{$gene_map_ref})){
        ...
        #if(($chrom_id==$chromosome_id)&&($strand eq "$tstrand")){
...
print "A total of ".keys(%strainFiles)."\n";
 while(($prediction,$samplefiles)=each(%strainFiles)){ #we process data by strain
    ...
    while(($sample,$file)=each(%{$samplefiles})){ 
        ...
        if($suffix){
           ...
           #print "Processing $orgv_id,$sample:$sample_id, $prediction,$sample,$file\n";
          ...
          if(IN){  
            ...
            #Indexes are created to detect identical transcripts accross organismVersion strain/samples --
   ...
   if($linecont=~/^(\d+)\s$rnaseqTxfile/){$linecont=$1;
      if($linecont>0){--$linecont;
        # $qh_deleternaseq->execute($orgv_id,$prediction_id);
  ...
  open(MAIN,">tempfile-$prediction.novelGenes.txt");$orgversion="";
  print MAIN "organism_version\ttranscript_id\tgene_name\tgene_prediction\n";
  open(db,">tempfile-$prediction-gene_synonyms.txt");
  print db "organism_version\tgene_prediction\tnovel_gene_name\tsynonym_gene_name\ttranscript_list\n";
  open(NOV,">tempfile-$prediction.novelGenesBYannot.txt");
   print NOV "orgv_id\tprediction_id\ttx_name\ttxid\tnovel_gene\n";
 ...
 loadNovelG(\%uniq_region_map,$orgv_id,$prediction_id);
  ...
  # print "total of $nof not found; total of $ovelap partial overlap; Total found: $exists\n";
   #now load novel_genes and novel_genes_synonyms tables for this strain
   $genfile="$dir/tempfile-$prediction.novelGenes.txt";$geneSynFile="$dir/tempfile-$prediction-gene_synonyms.txt";
   $geneByannot="$dir/tempfile-$prediction.novelGenesBYannot.txt";
