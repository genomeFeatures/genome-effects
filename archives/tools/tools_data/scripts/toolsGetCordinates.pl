...
###################################################################################################
## getCordinates.pl -- 
# generates  a genePred file with the following fields
...
if($opt_d){
  ...
  if($ucsc_dbh){$ucsc_dbh->{PrintError} = 0; # disable
     ...
     if(!$rv){print "$host does not have $gene_prediction annotations for $organism_version\n";}
     else{
       ...
       while(($name,$chrom,$strand,$txStart,$txEnd,$cdsStart,$cdsEnd,$exonCount,$exonStarts,$exonEnds,$name2)=$qh_ucscgetTxByAnnot->fetchrow_array()){
